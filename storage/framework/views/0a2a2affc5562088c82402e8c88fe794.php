...
<body>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="Enquiry AddForm" id="enquiry_type" enctype="multipart/form-data" novalidate>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control mt-1 inputfield" id="category_name"
                                        name="category_name" pattern="[^0-9]+" placeholder="Enter Category name"
                                        autofocus required>
                                </div>
                                <div class="form-group">
                        ...
                        <div class="row">
                            <div class=" col-12">
                                ...
                                <input type="text" class="form-control mt-1 inputfield" id="update_enquiry_name"
                                    name="update_enquiry_name" pattern="[^0-9]+" placeholder="Enter Category name"
                                    autofocus required>
                            </div>
                            <div class=" col-12">
                                <label class="mt-2 mb-1 inputlabel">Description:<span
                                ...
                                <input type="text" class="form-control mt-1 inputfield"
                                    id="update_enquiry_description" name="update_enquiry_description" pattern="[^0-9]+"
                                    placeholder="Enter description" autofocus required>
                            </div>
                            <div class="col-xl-6 col-lg-6 col-6">
                                <label class="mt-2 mb-1 inputlabel">Profile Picture</label><br>
                                <input type="file" class="form-control mt-1" id="update_enquiry_profile_picture"
                                    name="update_enquiry_profile_picture" accept="image/*">
                            </div>
                            <div class="form-group">
                                <label class="mt-2 mb-1 inputlabel">Type:<span
                                        style="color:red; font-size:15px">*</span></label><br>
                                <select class="form-control mt-1 inputfield" id="update_enquirytype" name="update_enquirytype" required>
                                    <option value="" disabled selected>Select Type</option>
                                    <option value="doctor">Doctor</option>
                                    <option value="medicine">Medicine</option>
                                    <option value="symptoms">Symptoms</option>
                            ...
                            <div class="form-group">
                                <label for="doctorsList">Select Doctor:</label>
                                <select class="form-control" id="update_enquirydoctorsList" name="update_enquirydoctorsList"></select>
                            </div>
                        </div>
    ...
    <script>
        ...
        //select from one details of docter
    ...
    $('#type').change(function() {
        var selectedType = $(this).val();
        if (selectedType === 'doctor' || selectedType === 'symptoms') {
            // Fetch data through AJAX
            $.ajax({
                url: '/api/getalldocters', // Assuming this endpoint returns doctors for both types
                method: 'GET',
                dataType: 'json',
                success: function(data) {
                    // Show and initialize selectize with fetched data
                    doctorsDropdown.empty(); // Clear previous options
                    data.Docters.forEach(function(doctor) {
                        doctorsDropdown.append('<option value="' + doctor.id + '">' + doctor.firstname + '</option>');
                    });
                    doctorsDropdown.show().selectize({
                        plugins: ['remove_button'],
                        create: false,
                        ...
                        labelField: 'firstname',
                        searchField: ['firstname'],
                        options: data.Docters,
                        render: {
                            option: function(item, escape) {
                                return '<div>' +
                                    '<span class="title">' + escape(item.firstname) + '</span>' +
                                    '</div>';
                            }
                        },
                        onChange: function(value) {
                            selectedDoctorsArray = [];
                            value.forEach(function(doctorId) {
                                var doctor = data.Docters.find(function(d) {
                                    return d.id == doctorId;
                                });
                                if (doctor) {
                                    selectedDoctorsArray.push({
            ...
            // Hide and destroy selectize if neither "doctor" nor "symptoms" is selected
            var selectize = doctorsDropdown[0].selectize;
            if (selectize) {
                selectize.destroy();
            }
...
$('#MasterTable').on('click', '.btn_edit', function() {
    ...
    var settings = {
        "url": "/api/categories/" + EditSpecialization + "",
        "method": "GET",
        "timeout": 0,
    };
    $.ajax(settings).done(function(response) {
        ...
        if (Specializationedit.success == true) {
            ...
            $('#enquiry_type').hide();
            $('#update_enquiry').show();
            var CoTypeDataArray = Specializationedit.category;
            var selectedDoctersData = Specializationedit.selectedDocters;

            for (const key in CoTypeDataArray) {
                var SpecializationName = CoTypeDataArray['category_name'];
                var SpecializationRemark = CoTypeDataArray['description'];
                // var SpecializationImage = CoTypeDataArray['image'];
                var SpecializationType = CoTypeDataArray['type'];
                var SpecializationdataList = selectedDoctersData['dataList'];
                var SpecializationId = CoTypeDataArray['id'];
            }

            $("#update_enquiry_id").val(SpecializationId);
            $("#update_enquiry_name").val(SpecializationName);
            $("#update_enquiry_description").val(SpecializationRemark);
            $("#update_enquirytype").val(SpecializationType);

            // Set doctor dropdown options
            var doctorsDropdown = $('#update_enquirydoctorsList');
            doctorsDropdown.show().html(''); // Clear previous options
            ...
            // Assuming data.Docters is an array of doctors
            doctorsDropdown.selectize({
                plugins: ['remove_button'],
                create: false,
                persist: false,
                maxItems: null,
                valueField: 'id',
                labelField: 'firstname',
                options: Specializationedit.Docters, // Assuming data.Docters is an array of doctors
                render: {
                    option: function(item, escape) {
                        return '<div>' +
                            '<span class="title">' + escape(item.firstname) + '</span>' +
            ...
            if (SpecializationdataList) {
                var selectedValues = SpecializationdataList.map(function(doctor) {
                    return doctor.id;
                });
                doctorsDropdown[0].selectize.setValue(selectedValues);
            }
        }
    });
});
        ...
        $("#update_enquiry").validate({
            ...
            submitHandler: function(form) {
                ...
                var UpdateRemark = $('#update_enquiry_description').val();
                var UpdateType = $('#update_enquirytype').val();
                var UpdatedDy = $('#updated_by').val();
                $.ajax({
                    url: "/api/Categories/" + UpdateId + "",
                    method: "PUT",
                    timeout: 0,
                    headers: {
                        "Accept": "application/json",
                    ...
                    data: {
                        category_name: UpdateSpecialization,
                        description: UpdateRemark,
                        type: UpdateType,
                    },
                    beforeSend: function() {
                        $('.loader').show();
...
<?php /**PATH /home/1163996.cloudwaysapps.com/tdtjxymxyy/public_html/resources/views/categories.blade.php ENDPATH**/ ?>